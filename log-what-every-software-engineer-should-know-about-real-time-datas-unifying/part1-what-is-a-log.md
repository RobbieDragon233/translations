...
当碰到需要以相同顺序获取同一数据时，应该触发你的条件反射：这个地方要引入日志，这就是日志的引入的原因。
下面是个很直觉的意识：如果给两段确定性代码相同的日志输入，那么它们就会生产相同的输出。

应用到分布式计算中就相当明显了。你可以把将协调多台机器执行相同任务的问题，归约为以一致性日志作为进程输入，达到协调进程的目的。
这里日志的目的是把所有非确定性的东西排除在输入流之外，以确保处理这些输入的各个副本（`replica`）保持同步。
...
这个过程也是可逆的：如果你对一张表进行更新，你可以记录这些变更，并把所有的更新以『变更日志』的形式发布到表的状态信息中。
